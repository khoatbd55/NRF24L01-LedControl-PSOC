...
#define nRF24_CSN_L SPI_NRF_CS_Write(0);
#define nRF24_CSN_H SPI_NRF_CS_Write(1);
...
uint8_t nRF24_LL_RW(uint8_t b)
{
    SPI_NRF_SpiUartClearTxBuffer();
    SPI_NRF_SpiUartClearRxBuffer();
    SPI_NRF_SpiUartWriteTxData(b);
    int32_t timeOutUs=1000;
    uint8_t result=0,count=1;
    do
    {
        if(SPI_NRF_SpiUartGetRxBufferSize()>0)
        {
            result=SPI_NRF_SpiUartReadRxData();
            count--;
        }
        CyDelayUs(1);
        timeOutUs--;
...
void nRF24_MasterInit(void) 
{
	nRF24_CSN_H;
		...
		// Configure RX PIPE#1
		static const uint8_t nRF24_ADDR[] = { 'K' ,'H' , 'A', 'N', 'G'};
		nRF24_CE_L;
		...
		nRF24_SetRXPipe(nRF24_PIPE0, nRF24_AA_OFF, 6); // enable RX pipe#1 with Auto-ACK: disabled, payload length: 6 bytes
		//=== end here
...
void nRF24_SlaveInit() {
	...
	static const uint8_t nRF24_ADDR[5] = { 'K' ,'H' , 'A', 'N' ,'G'};
	nRF24_SetPowerMode(nRF24_PWR_DOWN);
	nRF24_DisableAA(0xFF); // disable ShockBurst
	nRF24_SetRFChannel(115); // set RF channel to 2520MHzs
	nRF24_SetDataRate(nRF24_DR_250kbps); // 2Mbit/s data rate
	nRF24_SetCRCScheme(nRF24_CRC_2byte); // 1-byte CRC scheme
	nRF24_SetAddrWidth(5); // address width is 5 bytes
	nRF24_SetTXPower(nRF24_TXPWR_0dBm); // configure TX power
...
uint8_t nRF24_Check(void) {
	...
	//memcpy(ptr_bug ,ptr ,sizeof(nRF24_TEST_ADDR) - 1U);
